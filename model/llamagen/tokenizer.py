...
if __name__ == "__main__":
    ...
    model = VQ_16()
    ckpt = torch.load("/home/jiachun/ckpt/llamagen/vq_ds16_c2i.pt", map_location="cpu")["model"]
    # print(ckpt.keys())
    ...
    with torch.no_grad():
        ...
        # code = info[2]
        # code = rearrange(code, "(B L) -> B L", B=B)
        # print(code.shape)
        # print(code[0][0])
        # print(code[1][0])
        # code_2d = rearrange(code, "B (H W) -> B H W", H=16, W=16)
        # rec = model.decode_code(code_2d, shape=(1, 8, 16, 16))
        # dec = ((rec + 1) / 2).clamp(0, 1)
        # rec = ToPILImage()(dec[0])
        # rec.save("./rec_code.png")