...
def modify_internvl_parallel_head(internvl, config):
    """
    add parallel head to internvl
    """
    from model.mcq_gen.parallel_head import ParallelHead
    ...
    parallel_head = ParallelHead(config.head)
    parallel_head.requires_grad_(True)
    num_params = sum(p.numel() for p in parallel_head.parameters() if p.requires_grad)
    print(f"Trainable parameters in parallel_head: {num_params / 1e6:.2f}M")

    internvl.visual_projector = visual_projector
    internvl.parallel_head = parallel_head
...
class MyTrainer(Trainer):
    ...
    def _load_models(self):
        ...
        internvl = modify_internvl_parallel_head(internvl, self.config.model)
